    ...
    def merge_to(self, sd, dtype, device):
        ...
        if len(weight.size()) == 2:
            ...
            weight = weight + self.multiplier * \
                (up_weight @ down_weight) * self.scale
            ...
            # conv2d 3x3
            conved = torch.nn.functional.conv2d(down_weight.permute(
                1, 0, 2, 3), up_weight).permute(1, 0, 2, 3)
...
class LoRAInfModule(LoRAModule):
    ...
    def to_out_forward(self, x):
        ...
        has_real_uncond = x.size(
        )[0] // self.network.batch_size == self.network.num_sub_prompts + 2

        out = torch.zeros((self.network.batch_size * (3 if has_real_uncond else 2),
        ...
        for i in range(self.network.batch_size):
            # 1枚の画像ごとに処理する
            lx1 = lx[i * self.network.num_sub_prompts:
                     (i + 1) * self.network.num_sub_prompts]
            lx1 = lx1 * mask
...
def create_network(multiplier, network_dim, network_alpha, vae, text_encoder, unet, **kwargs):
    ...
    if block_dims is not None or down_lr_weight is not None or mid_lr_weight is not None or up_lr_weight is not None:
        ...
        if down_lr_weight is not None:
            ...
            if "," in down_lr_weight:
                down_lr_weight = [(float(s) if s else 0.0)
                                  for s in down_lr_weight.split(",")]
        ...
        if up_lr_weight is not None:
            if "," in up_lr_weight:
                up_lr_weight = [(float(s) if s else 0.0)
                                for s in up_lr_weight.split(",")]

        down_lr_weight, mid_lr_weight, up_lr_weight = get_block_lr_weight(
            down_lr_weight, mid_lr_weight, up_lr_weight, float(
                kwargs.get("block_lr_zero_threshold", 0.0))
        )
    ...
    if up_lr_weight is not None or mid_lr_weight is not None or down_lr_weight is not None:
        network.set_block_lr_weight(
            up_lr_weight, mid_lr_weight, down_lr_weight)
    ...
    if (up_lr_weight != None and len(up_lr_weight) < max_len) or (down_lr_weight != None and len(down_lr_weight) < max_len):
        ...
        if down_lr_weight != None and len(down_lr_weight) < max_len:
            down_lr_weight = down_lr_weight + \
                [1.0] * (max_len - len(down_lr_weight))
...
class LoRANetwork(torch.nn.Module):
    ...
    UNET_TARGET_REPLACE_MODULE_CONV2D_3X3 = [
        "ResnetBlock2D", "Downsample2D", "Upsample2D"]
    ...
    def __init__(
        ...
        self.text_encoder_loras, skipped_te = create_modules(
            False, text_encoder, LoRANetwork.TEXT_ENCODER_TARGET_REPLACE_MODULE)
        print(
        ...
        for lora in self.text_encoder_loras + self.unet_loras:
            ...
            for key in weights_sd.keys():
                if key.startswith(lora.lora_name):
                    sd_for_lora[key[len(lora.lora_name) + 1:]
                                ] = weights_sd[key]
    ...
    def get_lr_weight(self, lora: LoRAModule) -> float:
        ...
        if block_idx < LoRANetwork.NUM_OF_BLOCKS:
            ...
            if self.up_lr_weight != None:
                lr_weight = self.up_lr_weight[block_idx -
                                              LoRANetwork.NUM_OF_BLOCKS - 1]
                ...
                for idx, block_loras in block_idx_to_lora.items():
                    ...
                    if unet_lr is not None:
                        param_data["lr"] = unet_lr * \
                            self.get_lr_weight(block_loras[0])
    ...
    def set_current_generation(self, batch_size, num_sub_prompts, width, height, shared):
        ...
        def resize_add(mh, mw):
            ...
            m = torch.nn.functional.interpolate(
                mask, (mh, mw), mode="bilinear")  # doesn't work in bf16
